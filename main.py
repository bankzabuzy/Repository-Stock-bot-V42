...
def v1412_restore_clean(text):
    ...
    text = re.sub(r"Version\s*:\s*[^\n]+", "Version : " + V1412_RESTORE_VERSION, text)
    if "Version : " not in text:
        text = text.rstrip() + "\n\nVersion : " + V1412_RESTORE_VERSION
    return text.strip()

def v1412_restore_dispatch(user_text):
    try:
        from v1412_restore_full_analysis.commands.full_formatter import dispatch
        return dispatch(user_text)
    except Exception as e:
        return f"V1412 restore error: {e}\n\nVersion : {V1412_RESTORE_VERSION}"
...
def v1412_full_symbol_route(symbol_text):
    from v1412_restore_full_analysis.commands.full_formatter import analysis
    return Response(analysis(symbol_text), mimetype="text/plain; charset=utf-8")

@app.route("/v1412/top5/<kind>", methods=["GET"], endpoint="v1412_top5_kind")
def v1412_top5_kind_route(kind):
    from v1412_restore_full_analysis.commands.full_formatter import top5
    return Response(top5(kind), mimetype="text/plain; charset=utf-8")

@app.route("/v1412/health", methods=["GET"], endpoint="v1412_health")
def v1412_health_route():
    from v1412_restore_full_analysis.api.router import health_text
...
def v1414_clean(text):
    ...
    for old in [
        ...
        text = text.replace(old, V1414_VERSION)
    text = re.sub(r"Version\s*:\s*[^\n]+", "Version : " + V1414_VERSION, text)
    if "Version : " not in text:
        text = text.rstrip() + "\n\nVersion : " + V1414_VERSION
    return text.strip()

def v1414_dispatch(user_text):
    try:
        from v1413_worldclass_line_os.commands.worldclass_line_builder import dispatch
        return dispatch(user_text)
    except Exception as e:
        return f"V1414 error: {e}\n\nVersion : {V1414_VERSION}"

try:
    _v1414_prev_handle_line_command = handle_line_command
except Exception:
...
@app.route("/v1414/price/<symbol_text>", methods=["GET"], endpoint="v1414_price_symbol")
def v1414_price_symbol_route(symbol_text):
    import json
    from v1414_realtime_price_router.api.realtime_price_router import PriceRouter
    return Response(json.dumps(PriceRouter().quote(symbol_text), ensure_ascii=False, indent=2), mimetype="application/json; charset=utf-8")

@app.route("/v1414/health", methods=["GET"], endpoint="v1414_health")
def v1414_health_route():
    from v1413_worldclass_line_os.commands.worldclass_line_builder import dispatch
    return Response(dispatch("api"), mimetype="text/plain; charset=utf-8")
...
def v1414_1_clean(text):
    ...
    for old in [
        ...
        text = text.replace(old, V1414_1_VERSION)
    text = re.sub(r"Version\s*:\s*[^\n]+", "Version : " + V1414_1_VERSION, text)
    if "Version : " not in text:
        text = text.rstrip() + "\n\nVersion : " + V1414_1_VERSION
    return text.strip()

def v1414_1_dispatch(user_text):
    try:
        from v1413_worldclass_line_os.commands.worldclass_line_builder import dispatch
        return dispatch(user_text)
    except Exception as e:
        return f"V1414.1 error: {e}\n\nVersion : {V1414_1_VERSION}"

try:
    _v1414_1_prev_handle_line_command = handle_line_command
except Exception:
...
def v1414_1_status_route():
    ...
    return Response(unified_control_center(), mimetype="text/plain; charset=utf-8")

@app.route("/v1414_1/api/<symbol_text>", methods=["GET"], endpoint="v1414_1_api_symbol")
def v1414_1_api_symbol_route(symbol_text):
    from v1414_unified_status_control.commands.unified_status import symbol_api_status
    return Response(symbol_api_status(symbol_text), mimetype="text/plain; charset=utf-8")
