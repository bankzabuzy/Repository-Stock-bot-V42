...
from .common import init_db, connect, V220_VERSION
...
def compatibility_report(compile_ok=True) -> dict:
    init_db()
    ...
    report = {
        ...
        "imports": imports,
        "routes": routes,
        "compatibility": "V220 extends V210 with broker execution network while keeping V170-V210 endpoints",
    }
    try:
        conn = connect(); cur = conn.cursor()
        cur.execute("INSERT INTO v220_compatibility_audit(created_at,latest_version,compile_ok,route_collision_count,required_modules_ok,report) VALUES(?,?,?,?,?,?)",
                    (datetime.now(timezone.utc).isoformat(), V220_VERSION, 1 if compile_ok else 0, routes["collision_count"], 1 if imports["ok"] else 0, json.dumps(report, ensure_ascii=False, default=str)))
