...
def compatibility_report(compile_ok=True):
    ...
    report={"ok": imports["ok"], "version": V240_VERSION, "compile_ok": compile_ok, "imports": imports, "routes": routes}
    try:
        conn=connect(); cur=conn.cursor()
        cur.execute("INSERT INTO v240_audit(created_at,latest_version,compile_ok,route_collision_count,compatibility_ok,report) VALUES(?,?,?,?,?,?)",
                    (datetime.now(timezone.utc).isoformat(), V240_VERSION, 1 if compile_ok else 0, routes["collision_count"], 1 if imports["ok"] else 0, json.dumps(report, ensure_ascii=False, default=str)))
